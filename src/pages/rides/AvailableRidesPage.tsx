...
import { acceptRide, getAvailableRides } from "../../api/rides/rides";
...
export default function AvailableRidesPage() {
  ...
  const [actingRideId, setActingRideId] = useState<number | null>(null);
  ...
  const handleAcceptRide = async (ride: RideResponse) => {
    try {
      setActingRideId(ride.id);
      setError("");
      setSuccess("");
      await acceptRide(ride.id);
      setRides((prev) => prev.filter((item) => item.id !== ride.id));
  ...
  if (user?.role !== "DRIVER") {
    return (
      <Alert severity="info">
        Available rides are visible only to driver accounts.
      </Alert>
    );
  }
    ...
    <Stack spacing={3.5}>
      <Stack spacing={1}>
        ...
        <Typography color="text.secondary">
          Review incoming ride requests and accept the next one you want to
          drive.
        </Typography>
      </Stack>
        ...
        >
          <CardContent sx={{ p: { xs: 3, md: 4 } }}>
            <Stack spacing={1.75} alignItems="flex-start">
              ...
              <Typography color="text.secondary">
                New ride requests will appear here as soon as passengers create
                them.
              </Typography>
            </Stack>
          </CardContent>
                      ...
                      <TableCell align="right">
                        ...
                        >
                          ...
                          <Button
                            variant="contained"
                            disabled={
                              actingRideId === ride.id || ride.status !== "REQUESTED"
                            }
                            onClick={() => void handleAcceptRide(ride)}
                            sx={{ borderRadius: 1 }}
                          >
                            {actingRideId === ride.id ? "Accepting..." : "Accept ride"}
                          </Button>
                        </Stack>
                      </TableCell>
                    </TableRow>
